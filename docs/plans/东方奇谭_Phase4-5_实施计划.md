...
> **⚠ 现状注记（2026-09-14）**：本计划 5.1–5.3 的**地形（障碍物）部分已废弃并被整体移除**。
> 竹林 / 石柱 / 石碑三种外观、`spawn_obstacles()`、`Obstacles` 解析几何、
> `scripts/obstacle.gd`、`Config.OBSTACLE_*` 均已从代码中删除；
> 地图主题保留，但只负责底色 / 网格 / 边框 / 氛围粒子，不再影响玩法。
> 移除理由见《类土豆兄弟游戏开发规划.md》§8.2「为什么地形（障碍物）被移除」。
> 本文其余部分（原文照录的任务定义）保持原样，作为历史记录，**不要按 5.1–5.3 去实现地形**。
