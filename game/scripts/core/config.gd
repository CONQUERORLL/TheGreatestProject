...
## ============================================================
## 版本号 —— **语义化版本 `MAJOR.MINOR.PATCH`**，规则由用户 2026-09-19 定下：
##   · **PATCH（小版本）**  ← **数值调整**（改伤害/血量/价格/概率/倍率等，不动机制）
##   · **MINOR（中版本）**  ← **机制修改**（新增/改变系统、新玩法、UI 结构、重构、数据模型）
##   · **MAJOR（大版本）**  ← **仅当用户明确宣布「正式发布」时**才涨（涨到 1.0.0）
##
## ⚠️ 这是**唯一真值来源**：菜单右下角显示的版本号读这里（`main_menu._build_version_tag`）。
##    别在 UI 里另写一份字符串 —— 两处会漂移。
## ⚠️ 版本号**不随自动构建变化**，只由人工在提交代码时按上表规则修改。
##    涨版本号与改代码**在同一次提交里**完成（见 `docs/版本号规则.md`）。
##
## 回溯说明：项目自 2026-09-04 起步（100 个提交），已按上述规则把历史改动
## 全部折算进来，并采用「同一特性/同一轮的连续迭代合并为一次中版本」的**保守归并**口径。
## 完整折算表见 `docs/版本号规则.md`。
## ============================================================
const VERSION_MAJOR := 0
const VERSION_MINOR := 50
const VERSION_PATCH := 0

## 版本号显示串（唯一拼装点）。形如 `v0.50.0`。
static func version_string() -> String:
	return "v%d.%d.%d" % [VERSION_MAJOR, VERSION_MINOR, VERSION_PATCH]
...
## 场上掉落物硬上限（2026-09-19）：事件波 / 高收获构筑会把 loot 撒到拾取半径之外
## 长期积压（实测第 15 波 563 件 → fps 144 掉到 10）。
## 生成侧（`enemy._spawn_loot`）达到上限时**并入同类掉落**（累加数值，不结算）——
## 因为波末已不再自动回收（掉落留在原地、由玩家走位拾取），
## 「超限直接入账」会与那条设计冲突。节点数钉在上限内，收益一分不少也不提前进账。
const LOOT_MAX := 200
...
## 区域加成（§5.5.3）：**进入某区块时**一次性给玩家的**永久属性增益**。
##
## 【第 16 轮（2026-09-19）二次改造 —— 用户原话】「区域属性不要变成增加同化度了，
##   改成各种增益 buff」。旧口径发的是 `assim_<元素> +0.10`：同化度是**受击结算内部
##   的一条系数**，玩家在面板上看不到即时反馈，20 波下来还容易与角色特性/五行反应
##   叠成通胀。新口径直接发 `dmg_mult / as_mult / crit_ch / armor / regen` 这类
##   **面板可见**的属性，与道具/升级同一套加成语义。
##
## 【数值同源，不另写一份表（铁律 1）】发放内容 = `ZONE_BUFFS[元素].player`，
##   与地面那片圆形区域（`terrain_zone_for_wave`）给的玩家侧增益**完全一致**。
##   分工刻意保留：本机制是「进区首波一次性、**永久**」的养成轴（选择在哪个区深耕）；
##   地面圆是「有边界、**可进出**」的站位收益，且**敌我通用**。
##
## 【一个元素只发一次】`region_sequence()` 里 5 个元素各出现一次，所以 20 波走满 5 区
##   的累计永久收益就是五套增益各一份（伤害 +12% / 攻速 +12% / 暴击 +6% /
##   护甲 +3 / 回血 +1.2）。
static func area_bonus_player(element: String) -> Dictionary:
	return zone_buff(element).get("player", {})
...
##
## 【与 `area_bonus_player` 的分工，刻意并存】那条是**进区首波一次性、永久**的养成轴
##   （选择在哪个区深耕，数值同源取本表 `player` 那一档）；本机制是**有边界、可进出的
##   站位收益**，且**敌我通用**。两者语义不同，不合并。
...
##    ⚠️ 第 15 轮（2026-09-19）由 0.0 改为**降权而非归零**（用户原话「武器槽满了后，
##       武器不刷新了，这不合理，我想换武器咋办」）。旧值 0.0 的前提是「满槽时
##       买武器会被 `buy()` 拦下 = 死格」，那条前提已经不存在：满槽时点武器卡会弹
##       「换掉哪把」的选择面板（见 `shop_ui._open_weapon_swap`），货架摆武器是**有用的**。
##       仍要降权是因为满槽时换装的边际收益低于「补一件道具 / 升一级」。
##    ⚠️ 关键不在「降到多少」，而在**让出的份额去哪**：见 shop_ui._roll_one ——
##       腾出的比例**全部并进升级池**，不流向普通道具。这是「道具挤占核心构筑件」的根治点。
const WEAPON_CHANCE_SLOTS_FULL := 0.5    # 满槽但可换装（降权，不归零）
const WEAPON_CHANCE_SATURATED := 0.25    # 武器侧饱和（满槽 + 全是进化体）：换装收益最低
...
## ⚠️ 第 15 轮（2026-09-19）**只回调后半段**：W1/W4 一字不动，W8 起逐段抬回
##    （用户原话「BOSS 和后期怪物伤害过低」；第 14 轮整条曲线砍了 25~30%，
##      当时治的是「早期伤害过陡 → W5 阵亡」，那一段的宽松保留）。
##    末段 W20 = 4.42 回到第 11 轮标定值 —— 与 HP 曲线同为「端点不动」的对偶。
const WAVE_DMG_CURVE := [
	[1, 1.00], [4, 1.15], [8, 1.70], [12, 2.60], [16, 3.50], [20, 4.42],
]
