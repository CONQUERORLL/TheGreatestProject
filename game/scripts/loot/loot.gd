...
func _ready() -> void:
	add_to_group("loot")
	# ⚠️⚠️ z_index **必须 >= 0**：`Main` 自己用 `_draw()` 画背景（底色/网格/边框），
	#    而掉落物是 Main 的**子节点**。z_index = -1 会让它排在 Main 自身绘制**之前**
	#    → 被整片背景盖住，**完全看不见**（2026-09-19 用户反馈「并没有看到材料掉落」的根因；
	#    此前靠波末自动回收 + 强磁吸掩盖了这个问题）。
	#    取 0 = 与敌人同层（先后由树序决定），保证画在背景之上。
	z_index = 0
